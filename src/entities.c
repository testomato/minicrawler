...
#define ENTITY(name, code) { #name, 0, 0, code, }

struct entity_desc entities[] = {
	ENTITY(nbsp, ' '),
	ENTITY(iexcl, 161),
	ENTITY(cent, 162),
	ENTITY(pound, 163),
...
static struct entity_desc *find_name_entity(const char *name, const size_t name_len)
{
	...
	const unsigned hash_index = hash_val % HASH_SIZE;
	struct entity_desc *ed = hash_cache[hash_index];
	if (ed && name_len == ed->len && !memcmp(name, ed->name, name_len))
		return ed;
	for (unsigned i = 0; i < sizeof(entities)/sizeof(*entities); ++i) {
		if (!entities[i].len) {
			entities[i].len = strlen(entities[i].name);
			entities[i].hash = hash(entities[i].name, entities[i].len);
			const unsigned index = entities[i].hash % HASH_SIZE;
			if (!hash_cache[index])
				hash_cache[index] = &entities[i];
		}
		if (hash_val == entities[i].hash && name_len == entities[i].len && !memcmp(name, entities[i].name, name_len)) {
			hash_cache[hash_index] = &entities[i];
			return &entities[i];
		}
	}
	return NULL;
...
char *consume_entity(char *s, const char *end, int *code)
{
	...
	if (end - s > max_entity_len)
		end = &s[max_entity_len];
	...
	*code = 0;
	...
	char *entity_end = p;
	if (s[1] == '#') {
		const int base = s[2] == 'x' || s[2] == 'X' ? 16 : 10;
		char *start = base == 16 ? &s[3] : &s[2];
		const int c = *entity_end;
		*entity_end = 0;
		char *endptr = NULL;
		const long l_code = strtol(start, &endptr, base);
		*entity_end = c;
		if (endptr != entity_end)
			return s;
		if ((int)l_code != l_code)
			return s;
		*code = (int)l_code;
		return &entity_end[1];
	}
	else {
		struct entity_desc *ed = find_name_entity(&s[1], entity_end - &s[1]);
		if (ed) {
			*code = ed->code;
			return &entity_end[1];
		}
